<template>
  <el-card class="box-card">

    <div slot="header" class="clearfix">
      <span style="font-size: 120%; padding: 6px">
        <b>文件夹容量：</b>{{tabs.length}}/{{capacity}}
      </span>

      <el-tooltip effect="light" :content="localeMessage.popUpHint" placement="top">
        <el-button class="btn" type="primary" icon="el-icon-circle-plus" plain></el-button>
      </el-tooltip>

      <a href="https://github.com/GooZy/tab-folder" target="_blank">
        <img src="data:image/svg+xml;base64,"
             alt="github" style="float: right; padding-right: 10px" width="20"/>
      </a>
    </div>

    <div v-if="tabs.length === 0">
      {{localeMessage.popUpTitle}}
    </div>

    <div v-for="tab in tabs" class="text item">
      <el-container style="height: 45px; border: 1px solid #eee">
        <el-aside width="35px">
          <img :src="tab.iconUrl" :alt="tab.title" height="35" width="35" style="padding: 10% 10% 10% 0">
        </el-aside>
        <el-container>
          <el-header height="50%" style="padding-top: 1%">
            <b class="cut-text">{{tab.title}}</b>
            <el-button class="btn" type="danger" icon="el-icon-remove" plain></el-button>
          </el-header>
          <el-footer height="20%">
            <b>创建时间：</b>{{utils.unixTime2String(tab.createTimeUnix)}}
          </el-footer>
        </el-container>
      </el-container>
      <br>
    </div>

  </el-card>
</template>

<script>
  const DEFAULT_FOLDER_SIZE = 20;
  export default {
    name: 'app',
    data() {
      return {
        tabs: [{
          'url': 'http://www.baidu.com',
          'title': '百度',
          'iconUrl': chrome.runtime.getURL("images/multiple.png"),
          'createTimeUnix': 1592318558000
        }, {
          'url': 'https://developer.chrome.com/extensions/background_pages',
          'title': 'Manage Events with Background Scripts - Google Chrome',
          'iconUrl': 'https://www.google.com/images/icons/product/chrome-32.png',
          'createTimeUnix': 1592469191150
        }, {
          'url': 'http://www.baidu.com',
          'title': '百度2',
          'iconUrl': chrome.runtime.getURL("images/multiple.png"),
          'createTimeUnix': 1592318558000
        }, {
          'url': 'https://developer.chrome.com/extensions/background_pages',
          'title': '02Manage Events with Background Scripts - Google Chrome',
          'iconUrl': 'https://www.google.com/images/icons/product/chrome-32.png',
          'createTimeUnix': 1592469191150
        }],
        localeMessage: {
          "popUpTitle": "",
          "popUpHint": ""
        },
        capacity: DEFAULT_FOLDER_SIZE,
        count: 0,
        utils: {
          'unixTime2String': chrome.extension.getBackgroundPage().unixTime2String
        }
      }
    },
    created() {
      // data init
      // this.tabs = [];

      // get data from storage
      let tabs = null;
      let capacity = null;
      chrome.storage.sync.get(['tabs', 'capacity'], function (result) {
        tabs = result.tabs;
        capacity = result.capacity;
      });
      this.tabs = tabs == null ? this.tabs : tabs;
      this.capacity = capacity == null ? this.capacity : capacity;

      // init locale messages
      for (let key in this.localeMessage) {
        this.localeMessage[key] = chrome.i18n.getMessage(key);
      }
    },
    methods: {

    }
  }
</script>

<style>
  .box-card {
    width: 300px;
  }
  .btn {
    float: right;
    padding: 0 0 0 0;
    width: 25px;
    height: 20px;
  }
  .cut-text {
    width: 130px;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    display: block;
    float: left;
  }
</style>
